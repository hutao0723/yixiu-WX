...
Vue.prototype.clickFun = function (event, cb, obj) {
  try {
    ...
    Vue.http.post('http://embedlog.youfen666.com/embed/click', params).then((res) => {
      // 埋点成功
    }, (res) => {
      // 埋点失败
    });
...
router.beforeEach((to, from, next) => {
  try {
    setTimeout(() => {
      ...
      Vue.http.post('http://embedlog.youfen666.com/embed/access',{app_id, stayTime, action, url, referer}).then((res) => {
        // 埋点成功
      }, (res) => {
        // 埋点失败
      });
